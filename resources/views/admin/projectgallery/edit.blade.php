      ...
      <div class="flex flex-wrap gap-2 mb-2">
        @foreach($subImages as $image)
          <div class="relative">
            ...
            <input type="hidden" name="kept_images[]" value="{{ $image->id }}">
            <button type="button" onclick="deleteImage({{ $image->id }})" class="absolute top-0 right-0 bg-red-600 text-white rounded-full p-1 text-xs">X</button>
          </div>
        @endforeach
      </div>
...
function deleteImage(id) {
    if (confirm('Are you sure you want to delete this image?')) {
        ...
        form.method = 'POST';
        form.action = '/admin/projectgallery/image/' + id;
